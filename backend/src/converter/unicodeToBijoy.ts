...
import { isTableBlock, type DocBlock, type DocRun } from "../model/docModel";
 ...
 */
function normalizeNukta(text: string): string {
  return text
 ...
 */
export function bengaliDensity(text: string): number {
...
export interface ConversionResult {
  bijoyText: string;
  /** True if every Bengali character was successfully mapped. */
  fullyConverted: boolean;
  /** Any Bengali Unicode codepoints left unconverted (should normally be empty). */
...
function convertSegment(segment: string): string {
  ...
  return bnUnicode2ANSI(normalized);
}
 ...
 */
export function unicodeToBijoy(text: string): ConversionResult {
 ...
 */
export function convertBlocks(blocks: DocBlock[]): BlockConversionResult {
  const unconvertedChars = new Set<string>();

  const convertRun = (run: DocRun): DocRun => {
    const bijoyText = run.text
      .split("\n")
      .map((line) => convertSegment(line))
      .join("\n");
    const leftover = bijoyText.match(BENGALI_BLOCK_GLOBAL_REGEX);
    if (leftover) leftover.forEach((c) => unconvertedChars.add(c));
    return { ...run, text: bijoyText };
  };

  const convertedBlocks: DocBlock[] = blocks.map((block) => {
